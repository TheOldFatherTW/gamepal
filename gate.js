(function () {
  ...
  function savePersonal(token) {
    try { localStorage.setItem("gamepal.viewKey", token); } catch (e) {}
    try {
      const path = location.pathname.replace(/[^/]+$/, "") || "/";
      document.cookie = "gamepal.viewKey=" + encodeURIComponent(token) + "; path=" + path + "; max-age=31536000; SameSite=Lax";
    } catch (e) {}
  }
  ...
  function currentKey() {
    ...
    let stored = "";
    try { stored = localStorage.getItem("gamepal.viewKey") || ""; } catch (e) {}
    let cookie = "";
    try {
      const m = document.cookie.match(/(?:^|; )gamepal\.viewKey=([^;]*)/);
  ...
  function lockSheetPage(mask) {
    ...
    mask.addEventListener("touchmove", function (ev) {
      ...
      const scroller = node && node.closest && node.closest(".list-tag-body, .tag-picker-suggest, .play-log");
